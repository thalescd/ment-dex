    ...
    lines.forEach((line) => {
        ...
        if (matchAbility) {
            ...
            if (matchAlso1) {
                ...
                abilities[ability]["also"].push(
                    "ABILITY_" + also.replace(" ", "")
                );
                abilities[ability]["also"].push(
                    "ABILITY_" + also.replace(" ", "_")
                );
            } else if (matchAlso2) {
                isNew = false;
...
export function regexVanillaAbilitiesDescription(textAbilitiesIngameName, abilities) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (ability) {
            ...
            const matchAbilityIngameName = lines[i].match(
                /_ *\( *" *(.*)" *\) *,/i
            );
            if (matchAbilityIngameName) {
                const abilityIngameName = matchAbilityIngameName[1];

                abilities[ability]["ingameName"] = sanitizeString(
                    abilityIngameName
                ).replace("\n", " ");
            }
        }
        ...
        if (matchConversionDescription) {
            ...
            if (ability) {
                ...
                else conversionTable[conversionDescription].push(ability);
            } else {
                ...
                if (matchDescription) {
                    ...
                    if (conversionTable[conversionDescription] !== undefined) {
                        for (
                            let j = 0;
                            j < conversionTable[conversionDescription].length;
                            j++
                        )
                            abilities[
...
export function regexAbilitiesIngameName(textAbilitiesIngameName, abilities) {
    ...
    lines.forEach((line) => {
        ...
        if (matchAbility) {
            ...
            abilitySanitized =
                `ABILITY_${matchAbility[1].replace(/_/g, "")}`.toUpperCase();
        }

        if (ability in abilities) abilityFound = true;
        else if (abilitySanitized in abilities) abilitySanitizedFound = true;
    });
...
export function regexAbilitiesDescription(textAbilitiesDescription, abilities) {
    ...
    lines.forEach((line) => {
        if (!line.includes("#ifdef")) {
            ...
            if (matchAbility) {
                ...
                for (let i = 0; i < abilityArray.length; i++) {
                    if (abilityArray[i] in abilities) {
                        abilities[abilityArray[i]]["description"] = line
                            .trim()
                            .replaceAll("\\n", " ");
                    }
                }
            ...
            if (abilities[ability] !== undefined) abilityArray.push(ability);
    ...
    Object.keys(replaceAbilities).forEach((oldAbility) => {
        Object.keys(replaceAbilities[oldAbility]).forEach((newAbility) => {
            ...
            if (oldAbility in abilities && !(newAbility in abilities)) {
                ...
                abilities[newAbility]["ingameName"] =
                    sanitizeString(newAbility);
                abilities[newAbility]["description"] =
                    abilities[oldAbility]["description"];
