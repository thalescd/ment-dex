...
export function regexMovesDescription(textMovesDescription, moves) {
    ...
    lines.forEach((line) => {
        ...
        if (matchDesc) {
            ...
            if (moves[descToMove] !== undefined) move.push(descToMove);
            if (moves[`${descToMove}_S`] !== undefined)
                move.push(`${descToMove}_S`);
            if (moves[`${descToMove}_P`] !== undefined)
                move.push(`${descToMove}_P`);

            const sanitizedMove = descToMove
                .replace(/_/g, "")
                .replace("MOVE", "MOVE_");
            if (
                moves[sanitizedMove] !== undefined &&
                sanitizedMove !== descToMove
            )
                move.push(sanitizedMove);
        } else {
            if (line.length < 10) move = [];
            else if (move.length > 0) {
                for (let i = 0; i < move.length; i++)
                    moves[move[i]]["description"] = [
                        line.replaceAll("\\n", " "),
                    ];
            }
        }
...
export function regexMoves(textMoves, moves) {
    ...
    let idx = 0;
    const regex =
        /ACTUAL_PLA_MOVE_POWERS|BUFFED_LEECH_LIFE|GEN_6_POWER_NERFS|DARK_VOID_ACC_NERF|FROSTBITE|UNBOUND/;
    ...
    lines.forEach((line) => {
        ...
        if (line.includes(".power")) {
            ...
            const matchFlags = line.match(/FLAG_\w+/gi);
            if (matchFlags) {
...
export function regexVanillaMovesDescription(
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (move) {
            ...
            if (move === "MOVE_FAINTATTACK") move = "MOVE_FEINTATTACK";
            else if (move === "MOVE_HIJUMPKICK") move = "MOVE_JUMPKICK";
        }

        const matchConversionDescription =
            lines[i].match(/gMoveDescription_\w+/i);
        if (matchConversionDescription) {
            ...
            if (move) {
                ...
                else conversionTable[conversionDescription].push(move);
            } else {
                const matchDescription = lines[i].match(
                    /_ *\( *" *(.*)" *\) *;/i
                );
                if (matchDescription) {
                    const description = [
                        matchDescription[1].replaceAll("\\n", " "),
                    ];
                    if (conversionTable[conversionDescription] !== undefined) {
                        if (
                            moves[conversionTable[conversionDescription][0]] !==
                            undefined
                        ) {
                            for (
                                let j = 0;
                                j <
                                conversionTable[conversionDescription].length;
                                j++
                            )
                                moves[
                                    conversionTable[conversionDescription][j]
...
function normalizeMoves(moves) {
    for (const move of Object.keys(moves)) {
        if (moves[move]["power"] === undefined) moves[move]["power"] = 0;

        if (moves[move]["PP"] === undefined) moves[move]["PP"] = 0;

        if (moves[move]["type"] === undefined) moves[move]["type"] = "";

        if (moves[move]["accuracy"] === undefined) moves[move]["accuracy"] = 0;

        if (moves[move]["split"] === undefined) moves[move]["split"] = "";

        if (moves[move]["effect"] === undefined) moves[move]["effect"] = "";

        if (moves[move]["chance"] === undefined) moves[move]["chance"] = "";

        if (moves[move]["chance"] === undefined) moves[move]["chance"] = 0;

        if (moves[move]["target"] === undefined) moves[move]["target"] = "";

        if (moves[move]["flags"] === undefined) moves[move]["flags"] = [""];

        if (moves[move]["priority"] === undefined) moves[move]["priority"] = 0;

        if (moves[move]["Zpower"] === undefined) moves[move]["Zpower"] = 0;

        if (moves[move]["Zeffect"] === undefined) moves[move]["Zeffect"] = "";

        if (moves[move]["Zeffect"] === undefined) moves[move]["Zeffect"] = "";

        if (moves[move]["maxPower"] === undefined) moves[move]["maxPower"] = "";
    }
