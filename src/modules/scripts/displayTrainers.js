function appendTrainersToTable(key) {
    ...
    if (!trainers[zone][trainer]["rematch"]) {
        ...
        trainerSprite.className = `sprite${trainers[zone][trainer]["sprite"]}`;
        trainerSprite.src = getTrainerSpriteSrc(
            trainers[zone][trainer]["sprite"]
        );
        const trainerName = document.createElement("span");
        trainerName.innerText = trainers[zone][trainer]["ingameName"];
        ...
        trainerMainContainer.append(
            createTrainerSpeciesTbody(trainers[zone][trainer])
        );
    ...
    for (let i = 0; i < trainerObj["party"][difficulty].length; i++) {
        ...
        if (species[trainerSpeciesObj["name"]]["baseSpeed"] > 0) {
            ...
            trainerSpeciesAbility.innerText =
                abilities[
                    species[trainerSpeciesObj["name"]]["abilities"][
            ...
            trainerSpeciesAbility.addEventListener("click", () => {
                ...
                if (typeof innatesDefined !== "undefined") {
                    abilityArray = abilityArray.concat(
                        species[trainerSpeciesObj["name"]]["innates"]
                    );
                }
...
function returnMovesObj(trainerSpeciesObj) {
    ...
    for (let i = 0; i < trainerSpeciesObj["moves"].length; i++) {
        if (
            ...
            trainerSpeciesObj["moves"][i] in moves
        ) {
            ...
            trainerSpeciesMoveType.innerText = sanitizeString(
                moves[trainerSpeciesObj["moves"][i]]["type"]
            ).slice(0, 3);
            trainerSpeciesMoveType.className = `backgroundSmall ${moves[trainerSpeciesObj["moves"][i]]["type"]} trainersSpeciesMoveType`;
...
function createPopupItem(itemArray) {
    ...
    for (let i = 0; i < itemArray.length; i++) {
        ...
        itemName.innerText = `${items[itemArray[i]]["ingameName"]}: `;
        itemName.className = "bold";
        const itemDescription = document.createElement("span");
        itemDescription.innerText = items[itemArray[i]]["description"];
...
function replaceTbody(key, zone, trainer) {
    ...
    if (trainerEl) {
        ...
        trainerEl
            ...
            .replaceWith(createTrainerSpeciesTbody(trainers[zone][trainer]));
        const trainerFormat =
    ...
    for (let i = 0, j = trainersTracker.length; i < j; i++) {
        ...
        if (
            (trainers[zone][trainer]["rematch"] ||
                trainers[zone][trainer]["rematchArray"]) &&
            trainersTracker[i]["filter"].length === 0
        ) {
            let rememberI = i;
            let baseTrainer = trainer;
            if (trainers[zone][trainer]["rematch"]) {
            ...
            if (trainers[zone][baseTrainer]["rematchArray"]) {
                const rematchArray =
                    trainers[zone][baseTrainer]["rematchArray"].concat(
                        baseTrainer
                    );
                for (let k = 0; k < rematchArray.length; k++) {
                    if (
                        ...
                        rematchArray.includes(
                            trainersTracker[i - k]["key"].split("\\")[1]
                        )
                    ) {
