async function getScripts() {
    ...
    const rawScripts = await fetch(
        `https://raw.githubusercontent.com/${repo}/data/event_scripts.s`
    );
    const textScripts = await rawScripts.text();

    const rawTrade = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/trade.h`
    );
    const tradeText = await rawTrade.text();

    const rawSpecials = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/field_specials.c`
    );
...
async function getItems() {
    ...
    const rawItems = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/items.h`
    );
    ...
    const rawItemDescriptions = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/text/item_descriptions.h`
    );
...
async function getItemBallSripts(textScripts) {
    const rawItemBallSripts = await fetch(
        `https://raw.githubusercontent.com/${repo}/data/scripts/item_ball_scripts.inc`
    );
...
async function getHiddenItems() {
    const rawFlags = await fetch(
        `https://raw.githubusercontent.com/${repo}/include/constants/flags.h`
    );
...
async function getItemsIcon() {
    const rawItemIconTable = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/item_icon_table.h`
    );
    const textItemIconTable = await rawItemIconTable.text();

    const rawItemsIcon = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/graphics/items.h`
    );
...
async function getTrainers() {
    ...
    const rawTrainers = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/trainers.h`
    );
    const textTrainers = await rawTrainers.text();

    const rawTrainersParties = await fetch(
        `https://raw.githubusercontent.com/${repo}/src/data/trainer_parties.h`
    );
...
async function buildScriptsObjs() {
    ...
    */
    ...
    localStorage.setItem(
        "trainers",
        LZString.compressToUTF16(JSON.stringify(trainers))
    );
    localStorage.setItem(
        "items",
        LZString.compressToUTF16(JSON.stringify(items))
    );
    localStorage.setItem(
        "locations",
...
async function fetchScripts() {
    ...
    Object.keys(items).forEach(async (name) => {
        if (localStorage.getItem(`${name}`)) {
            sprites[name] = await LZString.decompressFromUTF16(
    ...
    Object.keys(trainers).forEach((zone) => {
        Object.keys(trainers[zone]).forEach((trainer) => {
            ...
            if (localStorage.getItem(sprite)) {
                sprites[sprite] = LZString.decompressFromUTF16(
                ...
                if (sprites[sprite].length < 500) {
                    ...
                    spriteRemoveBgReturnBase64(
                        sprite,
                        `https://raw.githubusercontent.com/${repo}/graphics/trainers/front_pics/${sprite.replace(/^TRAINER_PIC_/, "").toLowerCase()}_front_pic.png`
                    );
                }
            }
        });
    ...
    Object.keys(trainers).forEach((zone) => {
        Object.keys(trainers[zone]).forEach((trainer) => {
            if (!trainerToZone[trainer]) {
                ...
                if (!stop) {
                    Object.keys(trainers[trainerToZone[trainer]]).forEach(
                        (trainerName) => {
                            if (
                                trainerName.split("_").splice(0, 2).join("_") ==
                                    baseTrainerName &&
            ...
            if (correctZone) {
                if (correctZone === zone) {
                    if (
                        Object.keys(trainers[zone][trainer]["party"]).length ===
                        0
                    ) {
                        trainers[zone][trainer] = JSON.parse(
                            JSON.stringify(
                                trainers[trainerToZone[trainer]][trainer]
                            )
                        );
                    ...
                    if (
                        Object.keys(
                            trainers[trainerToZone[trainer]][trainer]["party"]
                        ).length === 0
                    ) {
                        trainers[trainerToZone[trainer]][trainer] = JSON.parse(
                            JSON.stringify(trainers[zone][trainer])
                        );
