...
function submitEnhancements() {
    ...
    const randomizedAbilitiesChecked = document.getElementById(
        "saveRandomizedAbilitiesCheckbox"
    ).checked;
    const randomizedLearnsetChecked = document.getElementById(
        "saveRandomizedLearnsetCheckbox"
    ).checked;
    const rebalancedStatsChecked = document.getElementById(
        "saveRebalancedStatsCheckbox"
    ).checked;
    let data = {};
    ...
    const saveRandomizedAbilities = settings.includes(
        "saveRandomizedAbilities"
    );
    ...
    if (
        (yidChanged &&
            (randomizedAbilitiesChecked || randomizedLearnsetChecked)) ||
        randomizedAbilitiesChecked != saveRandomizedAbilities ||
        randomizedLearnsetChecked != saveRandomizedLearnset ||
        rebalancedStatsChecked != saveRebalancedStats
    ) {
        clearSpeciesReload();
    } else {
        overlay.click();
...
async function reloadPopupEnhancements() {
        ...
        if (file) {
            ...
            file.arrayBuffer().then((buffer) => {
                ...
                if (trainerIdFull) {
                    ...
                    alert(
                        "Unable to read save data! Please ensure you've selected a save and not a save state."
                    );
                }
            });
        }
    ...
    saveOptionsFieldset.append(
        returnSaveSettingEl("saveRandomizedAbilities", "Randomized Abilities")
    );
    saveOptionsFieldset.append(
        returnSaveSettingEl("saveRandomizedLearnset", "Randomized Learnset")
    );
    saveOptionsFieldset.append(
        returnSaveSettingEl("saveRebalancedStats", "Rebalanced Stats")
    );
