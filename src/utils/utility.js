...
function exportData() {
    console.log(
        `let backupData = [${JSON.stringify(moves)}, ${JSON.stringify(abilities)}, ${JSON.stringify(species)}, ${JSON.stringify(locations)}, ${JSON.stringify(trainers)}, ${JSON.stringify(items)}, ${JSON.stringify(strategies)}, ${JSON.stringify(typeChart)}]`
    );
}
...
async function setDataList() {
    ...
    for (const abilityName in abilities) {
        if (
            ...
            !/[1-9aA-zZ]/.test(abilities[abilityName]["ingameName"])
        ) {
            continue;
        }
        const option = document.createElement("option");
...
function getSpeciesSpriteSrc(speciesName) {
    ...
    if (localStorage.getItem(speciesName)) {
        if (speciesName in sprites) {
            ...
            sprites[speciesName] = decodeSpriteDataString(
...
function returnTargetSpeciesSprite(speciesName) {
    if (
        species[speciesName]["forms"].length > 1 &&
        species[speciesName]["sprite"] ==
            species[species[speciesName]["forms"][0]]["sprite"]
    ) {
    ...
    if (speciesObj["type1"] !== speciesObj["type2"]) {
        if (typeof speciesObj["type3"] !== "undefined") {
            if (
                ...
                return (
                    typeChart[type][speciesObj["type1"]] *
                    typeChart[type][speciesObj["type2"]] *
                    typeChart[type][speciesObj["type3"]]
                );
            }
        } else {
            return (
...
function getPokemonEffectivenessValueAgainstType(speciesObj, type) {
    ...
    if (
        typeChart[speciesObj["type2"]][type] >
        typeChart[speciesObj["type1"]][type]
    ) {
        offensiveValue = typeChart[speciesObj["type2"]][type];
    }
    if (typeof speciesObj["type3"] !== "undefined") {
        if (
            typeChart[speciesObj["type3"]][type] >
                typeChart[speciesObj["type1"]][type] &&
            typeChart[speciesObj["type3"]][type] >
                typeChart[speciesObj["type2"]][type]
        ) {
